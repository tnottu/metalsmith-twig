...
function plugin(opts = {}) {
	...
	Twig.extendFilter('path', (assetPath) => {
		return `/${assetPath}`;
	});

	Twig.extendFilter('markdown', (plaintext) => {
		return commonmarkConvert(plaintext);
	});

	Twig.extendFilter('sanitize', (string) => {
		return slug(string, slug.defaults.modes.rfc3986);
	});
